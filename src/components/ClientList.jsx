  ...
  return (
    <div className="panel-left">
      ...
      <div className="client-list">
        {filtered.length === 0 && (
          <div className="empty-msg">Нет клиентов</div>
        )}
        {filtered.map(c => {
          ...
          return (
            <div
              key={c.id}
              className={`client-item ${c.id === selectedId ? 'selected' : ''}`}
            ...
            >
              <div className="client-row">
                ...
                <span className="state-badge" style={stateStyle(c.current_state)}>
                  {s.label}
                </span>
              </div>
              {c._lastMessage && (
                <div className="client-lastmsg">{c._lastMessage}</div>
              )}
